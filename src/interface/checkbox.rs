...
impl CheckBox {
    ...
    pub fn toggle(&mut self, mouse: &Mouse) {
        if self.is_mouse_over(mouse) && mouse.left_clicked() {
    ...
    pub fn draw(&self, mouse: &Mouse) {
        if self.pressed {
            self.draw_pressed(self.is_mouse_over(mouse));
        } else {
    ...
    fn draw_pressed(&self, hovered: bool) {
        set_drawing_colors(0x41);
        draw_rect(self.x, self.y, self.width, self.height);
        if hovered {
            set_drawing_colors(0x32);
